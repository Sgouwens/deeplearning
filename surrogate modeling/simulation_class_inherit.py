...
class SimulationBase:
    """Base class (to be inherited by more specialized training class)
    This class contains the ..."""

    def __init__(self, model, equation, equation_parameter, criterion, scheduler):

        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.criterion = criterion#nn.MSELoss()
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=initial_lr)
        self.scheduler = scheduler#torch.optim.lr_scheduler.LinearLR(self.optimizer, start_factor=0.01, end_factor=0.0001, total_iters=200)
