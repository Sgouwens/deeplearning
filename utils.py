...
def generate_B(n, k):
    indices = torch.randint(0, k, (n,))
    B = torch.ones((n, k), dtype=torch.float32)
    B[torch.arange(n), indices] = 0.0
    return B

def determine_helpers(batch, placeholder=placeholder_value):

    B = generate_B(*batch.shape) 
    Z = 1*torch.rand(batch.shape) # resembles Z in the paper Unif(0, 1)
    M = 1-torch.eq(batch, placeholder).float()
    H = B*M + .5*(1-B)
    return B, Z, H, M

def generate_imputed_batch(batch, placeholder=placeholder_value, train_generator=False):
    B, Z, H, M =  determine_helpers(batch, placeholder=placeholder)
    ...
    batch_discriminated = discriminate(batch_imputed, M) # hier stond eerst H ipv M..

    return B, H, M, Z, batch_imputed, batch_discriminated
